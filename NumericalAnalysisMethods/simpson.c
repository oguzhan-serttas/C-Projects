...
void simpson(struct Poly p, double a, double b, double n) {	//a alt sýnýr, b ust sýnýr, n yamuk sayýsý
	...
	answer += evaluate(p, b) + evaluate(p, a);
	
	printf("x\t\tf(x)\n");
	printf("%f\t%f\n", a, evaluate(p, a));
	
	for(i = 1; i <= n - 1; i= i + 2) {
		answer += 4 * evaluate(p, a + h * i);
	}
	
	for(i = 2; i <= n - 2; i= i + 2) {
		answer += 2 * evaluate(p, a + h * i);
	}
...
int main() {
	...
	printf("n degerini giriniz (n cift olmali !) : ");
	scanf("%lf", &n);
